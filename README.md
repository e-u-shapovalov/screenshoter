...
**Screenshoter** - маленькая программа для скриншотов на Windows. Она живет в системном трее, делает снимок выделенной области (в том числе с задержкой и обратным отсчётом, чтобы успеть поймать всплывающую подсказку), сохраняет PNG и сразу кладет в буфер обмена **и картинку, и путь к файлу**.
...
- поймать всплывающую подсказку или меню, которое исчезает при клике, с помощью режима задержки;
- получить путь к PNG-файлу для CLI, Markdown, редактора или системы заявок;
- вставить в чат только картинку, убрав путь из буфера одной клавишей;
- пользоваться скриншотером без аккаунта, облака, установщика и тяжелого интерфейса.

## Ключевые возможности

- Снимок выделенной области: `Ctrl+Shift+1`. В режиме выделения — яркий жёлтый курсор-перекрестье (виден на затемнённом фоне) и магнит к краям: рамка притягивается к ближайшей цветовой границе под курсором, чтобы ровно выделять кнопки, окна и блоки.
- Снимок с задержкой и обратным отсчётом: `Ctrl+Shift+3` — выделяете область, задаёте число секунд, успеваете навести курсор и поймать всплывающую подсказку (tooltip) или меню, которое исчезает при клике или нажатии клавиши.
- Переключатель пути в буфере: `Ctrl+Shift+2` — убрать путь из буфера (оставить только картинку для чатов) и вернуть его обратно, не делая новый снимок.
...
- Подсветка-вспышка по снятой области — сразу видно, что кадр сделан.
...
| Клавиши | Действие |
| --- | --- |
| `Ctrl+Shift+1` | Снимок области. Выделите прямоугольник мышью. Яркий жёлтый курсор-перекрестье хорошо виден на затемнённом фоне, а рамка магнитится к ближайшей цветовой границе под курсором — удобно ровно обводить кнопки, окна и блоки. `Esc` или правая кнопка мыши отменяют снимок. |
| `Ctrl+Shift+3` | Снимок с задержкой. Выделите область, укажите число секунд в окне, наведите курсор на нужный элемент — по окончании обратного отсчёта область снимется сама. Удобно для всплывающих подсказок и меню, которые исчезают при клике или нажатии клавиши. |
| `Ctrl+Shift+2` | Переключатель пути в буфере для последнего снимка. Первое нажатие убирает путь и оставляет только картинку (удобно для чатов вроде WhatsApp или ChatGPT, которые вставляют путь вместо изображения), повторное — возвращает путь обратно. Работает до следующего снимка, цветное уведомление показывает текущее состояние. |
...
- сделать снимок с задержкой;
- переключить путь в буфере (вкл/выкл);
...
Значит приложение, куда вы вставляете, предпочло текстовый формат. Нажмите `Ctrl+Shift+2` — программа уберёт путь из буфера и оставит только картинку, после чего `Ctrl+V` вставит изображение. Повторное нажатие `Ctrl+Shift+2` вернёт путь обратно. Также помогает вставка в графический редактор, Word, Paint, Telegram, Slack или другой чат.
