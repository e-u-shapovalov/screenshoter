...
- Страница релиза: [Screenshoter v1.2.0](https://github.com/e-u-shapovalov/screenshoter/releases/tag/v1.2.0).
- Готовый файл: [Screenshoter.exe](https://github.com/e-u-shapovalov/screenshoter/releases/download/v1.2.0/Screenshoter.exe).
- Размер asset в релизе: 44544 bytes.
- SHA256 asset `Screenshoter.exe` для `v1.2.0`: `51f61618dbefc315caabb2d33f900123f9f98409c9511688a4e234caa0646af8`.

Что нового в v1.2.0:

- В режиме выделения области (`Ctrl+Shift+1` и `Ctrl+Shift+3`) — **яркий жёлтый курсор-перекрестье** вместо системного серого, который терялся на затемнённом фоне.
- **Магнит к краям**: рамка притягивается к ближайшей цветовой границе под курсором (анализ пикселей замороженного снимка в рантайме) — удобно ровно обводить кнопки, окна и блоки. Притяжение мягкое: если выраженной границы рядом нет, координата остаётся точно под курсором.
